...
sns.scatterplot(x='X', y='Y', data=df_filt[df_filt.Object=='RNA_570'], hue='clusters', s=3, ax=ax)
ax.set_aspect('equal')
# %%
sub_df = df_filt[(df_filt.clusters == 3|2|1|0)]
dap = sub_df[sub_df.Object == 'DAPI']
rna = sub_df[sub_df.Object == 'RNA_570']
x_D, y_D = sns.kdeplot(dap.norm1D).lines[0].get_data()
x_r, y_r = sns.kdeplot(rna.norm1D).lines[1].get_data()
plt.show()
sns.scatterplot(x='X', y='Y', data=rna)
plt.show()

#%%
plt.scatter(x_r, y_r/y_D)
plt.show()
